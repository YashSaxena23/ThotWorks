...
public class SeedingApplicationDataConfiguration {
     ...
     *
     * @param builder
     * @return to serialize our pojo to json
     */
    @Bean
    @Primary
    public ObjectMapper objectMapper(Jackson2ObjectMapperBuilder builder) {
        ObjectMapper objectMapper = builder.createXmlMapper(false).build();
        objectMapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false); //state is false, hence date is not serialized
        //as timestamps. but textual -> INSTANCE here (epoch);
        return objectMapper;
    }
}
